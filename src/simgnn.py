...
class SimGNN(torch.nn.Module):
    ...
    def calculate_bottleneck_features(self):
        ...
        """
        if self.args.histogram == True: #如果需要直方图的话就是张量神经网络维度+直方图的维度
            self.feature_count = self.args.tensor_neurons + self.args.bins
        else:                           #否则的话就只是张量伸进网络的维度
    ...
    def setup_layers(self):
        ...
        self.convolution_1 = GCNConv(self.number_labels, self.args.filters_1)   #即三个GCN层，GCN是pyt自带的
        ...
        self.attention = AttentionModule(self.args)            #该函数是定义的。《注意力机制》
...
class SimGNNTrainer(object):
    ...
    def process_batch(self, batch):
        ...
        for graph_pair in batch:
            ...
            data = self.transfer_to_torch(data)#    关键预处理☆☆☆☆☆☆☆☆☆☆（已经one-hot编码的如何变成可以训练的格式）
    ...
    def fit(self):
        ...
        # -------------------pythrch固定语法-----
        self.model.train()
        epochs = trange(self.args.epochs, leave=True, desc="Epoch")#显示一个进度条
        #-----------------------pythrch固定语法---------------------------
